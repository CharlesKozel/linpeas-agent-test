### adapted from https://github.com/peass-ng/PEASS-ng/blob/master/parsers/peas2json.py
...
def is_redyellow(line: str) -> bool:
    """Returns a boolean

    Checks if line is colored redyellow and returns True or False
    """
    return line.find('\x1b[1;31;103m') > -1
...
def parse_peas_critical(peasOutput: str):
    critical_lines = []
    
    for line in peasOutput.splitlines():
        line = line.strip()
        if not line or not clean_colors(line):
            continue
        if is_redyellow(line):
            critical_lines.append(clean_title(clean_colors(line)))

    return critical_lines


def parse_peass(peasOutput: str):
